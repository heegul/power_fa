...
def _ensure_env_file(num_samples: int, num_pairs: int = 6):
    ...
    if os.path.exists(abs_path):
        return abs_path  # Already present – nothing to do.

    print(f"🛠  Environment file not found – generating {fname} ...")
    ...
    try:
        ...
        print("   ⚠️  Failed to generate environment file. Command output:")
        print(e.stdout)
        print(e.stderr)
        raise
...
class NetworkCapacityStudy:
    ...
    def _save_results(self):
        ...
        print(f"📁 Aggregated results saved to {self.results_file}")
    ...
    def run_study(self):
        ...
        print("🔬 Starting Network Capacity Study")
        ...
        print(f"✔︎ Preview: {len(self.results) - len(configs_to_run)} configurations will be skipped, {len(configs_to_run)} will be (re)run.")
    ...
    def run_experiment(self, hidden_config, n):
        ...
        if self.force_rerun and run_dirs:
            print(f"   🔄 FORCE RERUN enabled – deleting {len(run_dirs)} existing run directories for {exp_name}")
            for d in run_dirs:
                try:
                    shutil.rmtree(d)
        ...
        if num_completed >= num_runs_needed:
            print(f"✔︎ Skipping {exp_name}: {num_completed}/{num_runs_needed} runs already complete.")
        else:
            print(f"\n🚀 Running: {exp_name} ({num_completed}/{num_runs_needed} complete, running {num_runs_needed - num_completed} more)")
        ...
        print(f"   - ✅ Completed & Aggregated. {mem_msg}{gen_msg}")
        self._save_results()
    ...
    def run_generalization_only(self):
        ...
        print("🔍 Running GENERALIZATION-ONLY evaluation on existing models...")
        print("⚠️  WARNING: This will OVERRIDE any existing generalization results!")
        print(f"   Base directory: {self.base_dir}")
        ...
        print(f"✅ Found {len(existing_configs)} configurations with trained models")
        ...
        for config in existing_configs:
            ...
            print(f"\n🚀 Processing: {exp_name} ({len(model_dirs)} trained models)")
            ...
            for run_dir in model_dirs:
                ...
                if os.path.exists(gen_results_file):
                    print(f"   🔄 Run {run_id}: overriding existing generalization results")
                ...
                print(f"   🔍 Run {run_id}: running generalization (seed={unique_seed})")
                try:
                    subprocess.run(gen_cmd, check=True, capture_output=True, text=True)
                    ...
                    print(f"   ❌ Run {run_id}: generalization failed")
                    if e.stdout:
            ...
            if all_gen_ratios:
                ...
                print(f"   ✅ Updated with {len(final_gen_ratios)} generalization ratios")
                print(f"      Mean: {np.mean(final_gen_ratios):.4f} ± {np.std(final_gen_ratios):.4f}")
            else:
                print(f"   ❌ No successful generalization runs for {exp_name}")
        
        # Save updated results
        self._save_results()
        print(f"\n💾 Updated results saved to: {self.results_file}")
        print(f"🎉 Generalization-only evaluation complete!")
        print(f"   ✅ Processed {len(existing_configs)} configurations")
        print(f"   🔄 All generalization results have been re-computed with unique test data")

    def analyze_results(self, print_tables: bool = False):
        """Loads results and generates plots."""
        if not self.results:
            print("❌ No results to analyze. Please run the study first.")
            return
